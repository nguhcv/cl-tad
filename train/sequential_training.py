...
def reconstruction_learning(model_name,
                            R,
                            train_loader,
                            lr,
                            w,
    ...
    base_optimizer = torch.optim.Adam(list(R.parameters()), lr=lr)
    ...
    for e in range(n_epochs):
        ...
        for b_index, (data, label) in enumerate(train_loader):
            ...
            if type(R) is StackedSTINet:
                # data = data.permute(0, 2, 1)
                # label = label.permute(0, 2, 1)
                generated_batch = generated_batch.permute(0, 2, 1)
                generated_labels = generated_labels.permute(0, 2, 1)
            ...
            if (b_index + 1) % 16 == 0 or (b_index + 1) == len(train_loader):
                # update weights
                base_optimizer.step()
                # clear gradient
                base_optimizer.zero_grad()

                train_loss_sum += rc_loss.item()
...
def contrastive_learning(model_name,
                         F,
                         train_loader,
                         masking_factor,
                         lr,
    ...
    for e in range(n_epochs):
        ...
        for b_index, (data, label) in enumerate(train_loader):
            ...
            # calculate reconstruction-loss
            # ct_loss = ctloss_fn(ct_output, batch_output)
